...
template <typename T>
inline float row_inv_scale_w8a8(uint x_base, uint in_dim, const device T* x) {
    float max_abs = 0.0f;
    for (uint col = 0; col < in_dim; ++col) {
        float xv = float(x[x_base + col]);
        max_abs = max(max_abs, fabs(xv));
    }
    if (max_abs < 1e-8f) {
        return 1.0f;
    }
...
inline float qdq_w8a8(float xv, float inv_scale) {
    float q = round(xv * inv_scale);
    q = clamp(q, -127.0f, 127.0f);
    return q / inv_scale;
}

template <typename T>
inline void accum_iq2_xxs_word_w8a8(
    ulong grid_pack,
    uchar signs,
    float dl,
    uint base_col,
    ...
    for (uint j = 0; j < 8; ++j) {
        ...
        uchar gv = uchar((grid_pack >> (8 * j)) & 0xFF);
        float sgn = (signs & kmask_iq2xs[j]) ? -1.0f : 1.0f;
        float xq = qdq_w8a8(float(x[x_base + col]), inv_scale);
        acc += xq * (dl * float(gv) * sgn);
    }
}

template <typename T>
inline void accum_iq2_xs_word_w8a8(
    ushort qword,
    float dl,
    uint base_col,
    uint in_dim,
...
template <typename T>
inline void accum_iq2_s_grids_w8a8(
    uint grid_idx0,
    uint grid_idx1,
    uchar sign0,
    uchar sign1,
    ...
    for (uint j = 0; j < 8; ++j) {
        ...
        if (col0 < in_dim) {
            uchar gv0 = uchar((grid0 >> (8 * j)) & 0xFF);
            float sgn0 = (sign0 & kmask_iq2xs[j]) ? -1.0f : 1.0f;
            float xq = qdq_w8a8(float(x[x_base + col0]), inv_scale);
            acc += xq * (dl * float(gv0) * sgn0);
        }
        uint col1 = base_col + 8 + j;
        if (col1 < in_dim) {
            uchar gv1 = uchar((grid1 >> (8 * j)) & 0xFF);
            float sgn1 = (sign1 & kmask_iq2xs[j]) ? -1.0f : 1.0f;
            float xq = qdq_w8a8(float(x[x_base + col1]), inv_scale);
            acc += xq * (dl * float(gv1) * sgn1);
        }
    }
}
...
template <typename T>
[[kernel]] void iq2_xxs_matmul_w8a8_kernel(
    constant uint& m [[buffer(0)]],
    constant uint& out_dim [[buffer(1)]],
    constant uint& in_dim [[buffer(2)]],
    constant uint& blocks_per_row [[buffer(3)]],
    ...
    uint out_idx = gid % out_dim;
    uint x_base = row * in_dim;
    float inv_scale = row_inv_scale_w8a8<T>(x_base, in_dim, x);
...
template <typename T>
[[kernel]] void iq2_xs_matmul_w8a8_kernel(
    constant uint& m [[buffer(0)]],
    constant uint& out_dim [[buffer(1)]],
    constant uint& in_dim [[buffer(2)]],
    constant uint& blocks_per_row [[buffer(3)]],
    ...
    for (uint blk = 0; blk < blocks_per_row; ++blk) {
        ...
        for (uint ib32 = 0; ib32 < 8; ++ib32) {
            ...
            float dl0 = d * (0.5f + float(scale_byte & 0x0F)) * 0.25f;
            uint base0 = blk * 256 + ib32 * 32;
            accum_iq2_xs_word_w8a8(q20, dl0, base0 + 0, in_dim, x_base, inv_scale, x, acc);
            accum_iq2_xs_word_w8a8(q21, dl0, base0 + 8, in_dim, x_base, inv_scale, x, acc);

            float dl1 = d * (0.5f + float((scale_byte >> 4) & 0x0F)) * 0.25f;
            accum_iq2_xs_word_w8a8(q22, dl1, base0 + 16, in_dim, x_base, inv_scale, x, acc);
            accum_iq2_xs_word_w8a8(q23, dl1, base0 + 24, in_dim, x_base, inv_scale, x, acc);
        }
    }
...
template <typename T>
[[kernel]] void iq2_s_matmul_w8a8_kernel(
    constant uint& m [[buffer(0)]],
    constant uint& out_dim [[buffer(1)]],
    constant uint& in_dim [[buffer(2)]],
    constant uint& blocks_per_row [[buffer(3)]],
...
template <typename T>
[[kernel]] void iq3_s_matmul_w8a8_kernel(
    constant uint& m [[buffer(0)]],
    constant uint& out_dim [[buffer(1)]],
    constant uint& in_dim [[buffer(2)]],
    constant uint& blocks_per_row [[buffer(3)]],
...
inline float decode_iq2_xxs_value(
    const device uchar* w_bytes,
    const device uchar* w_scales,
    uint block_idx,
    uint local_col
) {
    uint byte_base = block_idx * 64;
    ...
    uint ib32 = local_col / 32;
    uint within32 = local_col % 32;
    uint g = within32 / 8;
    uint j = within32 % 8;
...
inline float decode_iq2_xs_value(
    const device uchar* w_bytes,
    const device uchar* w_scales,
    uint block_idx,
    uint local_col
    ...
    uchar scale_byte = w_scales[scale_base + 2 + ib32];
    float dl = (word < 2)
        ? d * (0.5f + float(scale_byte & 0x0F)) * 0.25f
        : d * (0.5f + float((scale_byte >> 4) & 0x0F)) * 0.25f;
...
inline float decode_iq2_s_value(
    const device uchar* w_bytes,
    const device uchar* w_scales,
    uint block_idx,
    uint local_col
    ...
    if (within16 < 8) {
        uchar qs0 = qs[qs_off + 0];
        sign_byte = qs[32 + qs_off + 0];
        grid_idx = uint(qs0) | ((uint(qh_nib) << 8) & 0x300);
    } else {
        uchar qs1 = qs[qs_off + 1];
        sign_byte = qs[32 + qs_off + 1];
        grid_idx = uint(qs1) | ((uint(qh_nib) << 6) & 0x300);
    }
    ulong grid = iq2s_grid[grid_idx];
    uchar gv = uchar((grid >> (8 * j)) & 0xFF);
    float sgn = (sign_byte & kmask_iq2xs[j]) ? -1.0f : 1.0f;
    return dl * float(gv) * sgn;
}

inline float decode_iq3_s_value(
    const device uchar* w_bytes,
    const device uchar* w_scales,
    uint block_idx,
    uint local_col
...
[[host_name("iq2_xxs_dequantize_f16")]] [[kernel]] void iq2_xxs_dequantize_f16_kernel(
    constant uint& out_dim [[buffer(0)]],
    constant uint& in_dim [[buffer(1)]],
    constant uint& blocks_per_row [[buffer(2)]],
    const device uchar* w_bytes [[buffer(3)]],
    const device uchar* w_scales [[buffer(4)]],
    device half* out [[buffer(5)]],
    uint gid [[thread_position_in_grid]]
) {
    uint n = out_dim * in_dim;
    if (gid >= n) {
        return;
    }
    uint out_idx = gid / in_dim;
    uint col = gid % in_dim;
    uint blk = col / 256;
    uint local_col = col % 256;
    uint block_idx = out_idx * blocks_per_row + blk;
    out[gid] = half(decode_iq2_xxs_value(w_bytes, w_scales, block_idx, local_col));
}

[[host_name("iq2_xs_dequantize_f16")]] [[kernel]] void iq2_xs_dequantize_f16_kernel(
    constant uint& out_dim [[buffer(0)]],
    constant uint& in_dim [[buffer(1)]],
    constant uint& blocks_per_row [[buffer(2)]],
    const device uchar* w_bytes [[buffer(3)]],
    ...
    uint local_col = col % 256;
    uint block_idx = out_idx * blocks_per_row + blk;
    out[gid] = half(decode_iq2_xs_value(w_bytes, w_scales, block_idx, local_col));
}

[[host_name("iq2_s_dequantize_f16")]] [[kernel]] void iq2_s_dequantize_f16_kernel(
    constant uint& out_dim [[buffer(0)]],
    constant uint& in_dim [[buffer(1)]],
    constant uint& blocks_per_row [[buffer(2)]],
    const device uchar* w_bytes [[buffer(3)]],
    ...
    uint local_col = col % 256;
    uint block_idx = out_idx * blocks_per_row + blk;
    out[gid] = half(decode_iq2_s_value(w_bytes, w_scales, block_idx, local_col));
}

[[host_name("iq3_s_dequantize_f16")]] [[kernel]] void iq3_s_dequantize_f16_kernel(
    constant uint& out_dim [[buffer(0)]],
    constant uint& in_dim [[buffer(1)]],
    constant uint& blocks_per_row [[buffer(2)]],
    const device uchar* w_bytes [[buffer(3)]],
    ...
    uint local_col = col % 256;
    uint block_idx = out_idx * blocks_per_row + blk;
    out[gid] = half(decode_iq3_s_value(w_bytes, w_scales, block_idx, local_col));
}
...
template [[host_name("iq2_xxs_matmul_w8a8_f32")]] [[kernel]]
decltype(iq2_xxs_matmul_w8a8_kernel<float>) iq2_xxs_matmul_w8a8_kernel<float>;
template [[host_name("iq2_xxs_matmul_w8a8_f16")]] [[kernel]]
decltype(iq2_xxs_matmul_w8a8_kernel<half>) iq2_xxs_matmul_w8a8_kernel<half>;
template [[host_name("iq2_xxs_matmul_w8a8_bf16")]] [[kernel]]
decltype(iq2_xxs_matmul_w8a8_kernel<bfloat>) iq2_xxs_matmul_w8a8_kernel<bfloat>;
...
template [[host_name("iq2_xs_matmul_w8a8_f32")]] [[kernel]]
decltype(iq2_xs_matmul_w8a8_kernel<float>) iq2_xs_matmul_w8a8_kernel<float>;
template [[host_name("iq2_xs_matmul_w8a8_f16")]] [[kernel]]
decltype(iq2_xs_matmul_w8a8_kernel<half>) iq2_xs_matmul_w8a8_kernel<half>;
template [[host_name("iq2_xs_matmul_w8a8_bf16")]] [[kernel]]
decltype(iq2_xs_matmul_w8a8_kernel<bfloat>) iq2_xs_matmul_w8a8_kernel<bfloat>;
...
template [[host_name("iq2_s_matmul_w8a8_f32")]] [[kernel]]
decltype(iq2_s_matmul_w8a8_kernel<float>) iq2_s_matmul_w8a8_kernel<float>;
template [[host_name("iq2_s_matmul_w8a8_f16")]] [[kernel]]
decltype(iq2_s_matmul_w8a8_kernel<half>) iq2_s_matmul_w8a8_kernel<half>;
template [[host_name("iq2_s_matmul_w8a8_bf16")]] [[kernel]]
decltype(iq2_s_matmul_w8a8_kernel<bfloat>) iq2_s_matmul_w8a8_kernel<bfloat>;
...
template [[host_name("iq3_s_matmul_w8a8_f32")]] [[kernel]]
decltype(iq3_s_matmul_w8a8_kernel<float>) iq3_s_matmul_w8a8_kernel<float>;
template [[host_name("iq3_s_matmul_w8a8_f16")]] [[kernel]]
decltype(iq3_s_matmul_w8a8_kernel<half>) iq3_s_matmul_w8a8_kernel<half>;
template [[host_name("iq3_s_matmul_w8a8_bf16")]] [[kernel]]
decltype(iq3_s_matmul_w8a8_kernel<bfloat>) iq3_s_matmul_w8a8_kernel<bfloat>;
